...
local list = view:new()
list.__index = list
function list:new(x, y, width, height, gravity)
    --独有属性
    ...
    instance.contentItems = {}                     --子视图
    local gap = instance.gap                       --元素间隙
    local itemHeight = instance.itemHeight         --元素高度
    for i = 1, 5 do
        local y = (i - 1) * itemHeight + (i * gap) -- 垂直位置
        local height = itemHeight                  -- 方块高度
        --print(y)
        table.insert(instance.contentItems, {
            x = 0,
    ...
    --设置元素总高
    local i = #instance.contentItems
    instance.contentHeight = gap + ((i - 1) * gap) + i * itemHeight --设置内容高度
    -- print(instance.contentHeight)
    return instance
end
...
function list:_on_click(id, x, y, dx, dy, istouch, pre) --拦截点击事件
    -- body
    local items = self.contentItems                     --元素总表
